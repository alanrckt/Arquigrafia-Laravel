  ...
  <div class="container">
    ...
    <div class="registration">
      ...
      <div class="three columns offset-by-one">
        ...
        <h1>Login com STOA</h1>
        <!-- <p>Temporariamente indisponível.</p> -->
        {{ Form::open() }}
          
          <p>Entre com seu usuário e senha:</p>
          <br>
          <div class="one columns alpha">{{ Form::label('login-stoa', 'Usuário:', array('class'=>'right')) }}</div>
          <div class="two columns omega">{{ Form::text('login-stoa', '', array('class'=>'right') ) }}</div>
          {{ $errors->first('login') }}
          
          <div class="one columns alpha">{{ Form::label('password-stoa', 'Senha:', array('class'=>'right')) }}</div>
          <div class="two columns omega">{{ Form::password('password-stoa', array('class'=>'right') ) }}</div>
